...
def accession_uniprot_cazy(accession_nb):
    """Extraction des numéros d'accession d'uniprot
    qui correspondent aux IDs de cazy db"""
    #Lecture du fichier excel
    global file1
    #file1 = input("Veuillez enter le nom du fichier ou le cemin d'accès : ")
    file1 = '/home/guest/Documents/Cornelia/cds_project/GH62/id_mapping/id_map_GH62_unKB.xlsx'
    read_file = pd.read_excel(file1)

    #Extraction des numéros d'accession
    accession_numbers = read_file['Entry'].tolist()
    accession_cazy = read_file['From'].tolist()
    #dictionnaire clé= accession_uniprot et valeur = accession_cazy
    dico_accession = dict(zip(accession_numbers, accession_cazy))
    #recupérer l'accession cazy
    cazy_accession = dico_accession[accession_nb]
    #print('\n','Accession cazy correspondant à ', accession_nb , ' = ', cazy_accession )
    #creation d'un dataframe
    df = pd.DataFrame({'From': accession_cazy, 'Entry': accession_numbers})
    df.to_csv('accession_uni_cazy.txt', sep='\t', index = False)
    return accession_numbers, accession_cazy, dico_accession
    #/home/guest/Documents/Cornelia/cds_project/GH62/id_mapping/id_map_GH62_unKB.xlsx

def get_accession_cz(accession_nb):
    results = accession_uniprot_cazy(accession_nb)
    dico_accession = results[2]
    #recupérer l'accession cazy
    cazy_accession = dico_accession[accession_nb]
    print('\n', 'Accession cazy correspondant à ', accession_nb, ' = ', cazy_accession)
...
if __name__ == '__main__' :
    ...
    # accession_nb_uni50()
    accession_nb = ['6CC7']
    for id in accession_nb :
        #accession_uniprot_cazy(id)
        get_accession_cz(id)
