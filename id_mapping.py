...
def accession_nb_uniref50():
    ...
    df.to_excel('accession_numbers_uniref50.xlsx', index=False)
    # # Lecture de la liste d'accessions à partir du fichier 'accession_numbers.txt'
    # with open('accession_numbers.txt', 'r') as f:
    #     accession_numbers = [line.strip() for line in f]
    #
    # # Construction de la requête HTTP pour l'ID mapping de UniprotKB AC/ID vers Uniref50
    # url = 'https://rest.uniprot.org/idmapping/run'
    # #url = 'https://www.uniprot.org/uploadlists/'
    # params = {
    #     'from': 'UniProtKB_AC-ID',
    #     'to': 'UniRef50',
    #     'format': 'tab',
    #     'query': ' '.join(accession_numbers)
    # }
    # response = requests.get(url, params=params)
    #
    # # Chargement des résultats dans un DataFrame pandas
    # df = pd.read_csv(response.content, sep='\t')
    #
    # # Écriture des résultats dans un fichier Excel
    # df.to_excel('id_mapping_results.xlsx', index=False)

if __name__ == '__main__' :
    accession_nb_list()
    get_repository()
    accession_nb_uni50()
    accession_nb_uniref50()
